...
class Block implements KeyListener {
  ...
  constructor(x: number, y: number, shape: number[][]) {
    ...
    this.color = Color.getRandomColorCode();
  ...
  public draw() {
    this.context.ctx.fillStyle = Color.getColor(this.color);
  ...
  onKeyDown(event: KeyboardEvent): void {
    ...
    switch (event.key) {
      case KeyboardKey.RIGHT:
        if (arena?.checkCollideWithWall(this) !== 1) this.moveRight();
        break;
      case KeyboardKey.LEFT:
        if (arena?.checkCollideWithWall(this) !== -1) this.moveLeft();
        break;
      case KeyboardKey.DOWN:
        if (!arena?.checkCollide(this)) this.moveDown();
        break;
      case KeyboardKey.UP:
        this.rotate();
        break;
